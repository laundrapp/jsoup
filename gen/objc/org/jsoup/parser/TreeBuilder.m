...
//  source: /Users/marcussmith/temp/jsoup/src/main/java/org/jsoup/parser/TreeBuilder.java
//
...
- (void)initialiseParseWithNSString:(NSString *)input
  ...
  JreStrongAssignAndConsume(&doc_, new_OrgJsoupNodesDocument_initWithNSString_(baseUri));
  JreStrongAssignAndConsume(&reader_, new_OrgJsoupParserCharacterReader_initWithNSString_(input));
  JreStrongAssign(&self->errors_, errors);
  JreStrongAssignAndConsume(&tokeniser_, new_OrgJsoupParserTokeniser_initWithOrgJsoupParserCharacterReader_withOrgJsoupParserParseErrorList_(reader_, errors));
  JreStrongAssignAndConsume(&stack_, new_JavaUtilArrayList_initWithInt_(32));
...
- (void)runParser {
  while (true) {
    ...
    [((OrgJsoupParserToken *) nil_chk(token)) reset];
...
- (jboolean)processStartTagWithNSString:(NSString *)name {
  if (currentToken_ == start_) {
    return [self processWithOrgJsoupParserToken:[create_OrgJsoupParserToken_StartTag_init() nameWithNSString:name]];
  }
...
- (jboolean)processStartTagWithNSString:(NSString *)name
  ...
  if (currentToken_ == start_) {
    return [self processWithOrgJsoupParserToken:[create_OrgJsoupParserToken_StartTag_init() nameAttrWithNSString:name withOrgJsoupNodesAttributes:attrs]];
  }
  [((OrgJsoupParserToken_StartTag *) nil_chk(start_)) reset];
  [((OrgJsoupParserToken_StartTag *) nil_chk(start_)) nameAttrWithNSString:name withOrgJsoupNodesAttributes:attrs];
  return [self processWithOrgJsoupParserToken:start_];
}

- (jboolean)processEndTagWithNSString:(NSString *)name {
  if (currentToken_ == end_) {
    return [self processWithOrgJsoupParserToken:[create_OrgJsoupParserToken_EndTag_init() nameWithNSString:name]];
  }
...
void OrgJsoupParserTreeBuilder_init(OrgJsoupParserTreeBuilder *self) {
  NSObject_init(self);
  JreStrongAssignAndConsume(&self->start_, new_OrgJsoupParserToken_StartTag_init());
  JreStrongAssignAndConsume(&self->end_, new_OrgJsoupParserToken_EndTag_init());
}
