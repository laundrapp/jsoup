...
//  source: /Users/marcussmith/temp/jsoup/src/main/java/org/jsoup/parser/Tokeniser.java
//
...
- (OrgJsoupParserToken *)read {
  ...
  if ([((JavaLangStringBuilder *) nil_chk(charsBuilder_)) java_length] > 0) {
    ...
    [((JavaLangStringBuilder *) nil_chk(charsBuilder_)) delete__WithInt:0 withInt:[charsBuilder_ java_length]];
    JreStrongAssign(&charsString_, nil);
...
- (void)emitWithOrgJsoupParserToken:(OrgJsoupParserToken *)token {
  ...
  JreStrongAssign(&emitPending_, token);
  isEmitPending_ = true;
  if (((OrgJsoupParserToken *) nil_chk(token))->type_ == JreLoadEnum(OrgJsoupParserToken_TokenType, StartTag)) {
    ...
    JreStrongAssign(&lastStartTag_, startTag->tagName_);
...
- (void)emitWithNSString:(NSString *)str {
  if (charsString_ == nil) {
    JreStrongAssign(&charsString_, str);
  }
  else {
    if ([((JavaLangStringBuilder *) nil_chk(charsBuilder_)) java_length] == 0) {
      [((JavaLangStringBuilder *) nil_chk(charsBuilder_)) appendWithNSString:charsString_];
    }
    [((JavaLangStringBuilder *) nil_chk(charsBuilder_)) appendWithNSString:str];
  }
}
...
- (IOSCharArray *)consumeCharacterReferenceWithJavaLangCharacter:(JavaLangCharacter *)additionalAllowedCharacter
  ...
  if ([((OrgJsoupParserCharacterReader *) nil_chk(reader_)) matchConsumeWithNSString:@"#"]) {
    ...
    if (!found) {
      [((OrgJsoupParserCharacterReader *) nil_chk(reader_)) rewindToMark];
      if (looksLegit) OrgJsoupParserTokeniser_characterReferenceErrorWithNSString_(self, NSString_java_formatWithNSString_withNSObjectArray_(@"invalid named referenece '%s'", [IOSObjectArray arrayWithObjects:(id[]){ nameRef } count:1 type:NSObject_class_()]));
      return nil;
    }
    if (inAttribute && ([((OrgJsoupParserCharacterReader *) nil_chk(reader_)) matchesLetter] || [((OrgJsoupParserCharacterReader *) nil_chk(reader_)) matchesDigit] || [((OrgJsoupParserCharacterReader *) nil_chk(reader_)) matchesAnyWithCharArray:[IOSCharArray arrayWithChars:(jchar[]){ '=', '-', '_' } count:3]])) {
      [((OrgJsoupParserCharacterReader *) nil_chk(reader_)) rewindToMark];
      return nil;
    }
    if (![((OrgJsoupParserCharacterReader *) nil_chk(reader_)) matchConsumeWithNSString:@";"]) OrgJsoupParserTokeniser_characterReferenceErrorWithNSString_(self, @"missing semicolon");
...
- (OrgJsoupParserToken_Tag *)createTagPendingWithBoolean:(jboolean)start {
  JreStrongAssign(&tagPending_, start ? [((OrgJsoupParserToken_StartTag *) nil_chk(startPending_)) reset] : [((OrgJsoupParserToken_EndTag *) nil_chk(endPending_)) reset]);
  return tagPending_;
}
...
- (void)createCommentPending {
  [((OrgJsoupParserToken_Comment *) nil_chk(commentPending_)) reset];
}
...
- (void)createDoctypePending {
  [((OrgJsoupParserToken_Doctype *) nil_chk(doctypePending_)) reset];
}
...
- (void)errorWithOrgJsoupParserTokeniserState:(OrgJsoupParserTokeniserState *)state {
  if ([((OrgJsoupParserParseErrorList *) nil_chk(errors_)) canAddError]) [((OrgJsoupParserParseErrorList *) nil_chk(errors_)) addWithId:create_OrgJsoupParserParseError_initWithInt_withNSString_withNSObjectArray_([((OrgJsoupParserCharacterReader *) nil_chk(reader_)) pos], @"Unexpected character '%s' in input state [%s]", [IOSObjectArray arrayWithObjects:(id[]){ JavaLangCharacter_valueOfWithChar_([((OrgJsoupParserCharacterReader *) nil_chk(reader_)) current]), state } count:2 type:NSObject_class_()])];
}

- (void)eofErrorWithOrgJsoupParserTokeniserState:(OrgJsoupParserTokeniserState *)state {
  if ([((OrgJsoupParserParseErrorList *) nil_chk(errors_)) canAddError]) [((OrgJsoupParserParseErrorList *) nil_chk(errors_)) addWithId:create_OrgJsoupParserParseError_initWithInt_withNSString_withNSObjectArray_([((OrgJsoupParserCharacterReader *) nil_chk(reader_)) pos], @"Unexpectedly reached end of file (EOF) in input state [%s]", [IOSObjectArray arrayWithObjects:(id[]){ state } count:1 type:NSObject_class_()])];
}
...
- (NSString *)unescapeEntitiesWithBoolean:(jboolean)inAttribute {
  ...
  while (![((OrgJsoupParserCharacterReader *) nil_chk(reader_)) isEmpty]) {
    [builder appendWithNSString:[((OrgJsoupParserCharacterReader *) nil_chk(reader_)) consumeToWithChar:'&']];
    if ([((OrgJsoupParserCharacterReader *) nil_chk(reader_)) matchesWithChar:'&']) {
      ...
      if (c == nil || c->size_ == 0) [builder appendWithChar:'&'];
      else [builder appendWithCharArray:c];
    }
  }
...
- (void)dealloc {
  ...
  RELEASE_(errors_);
  RELEASE_(state_);
  RELEASE_(emitPending_);
  RELEASE_(charsString_);
  RELEASE_(charsBuilder_);
  RELEASE_(dataBuffer_);
  RELEASE_(tagPending_);
  RELEASE_(startPending_);
  RELEASE_(endPending_);
  RELEASE_(charPending_);
  RELEASE_(doctypePending_);
  RELEASE_(commentPending_);
  RELEASE_(lastStartTag_);
  RELEASE_(charRefHolder_);
  [super dealloc];
}
...
+ (void)initialize {
  if (self == [OrgJsoupParserTokeniser class]) {
    JreStrongAssignAndConsume(&OrgJsoupParserTokeniser_notCharRefCharsSorted, [IOSCharArray newArrayWithChars:(jchar[]){ 0x0009, 0x000a, 0x000d, 0x000c, ' ', '<', '&' } count:7]);
    {
...
void OrgJsoupParserTokeniser_initWithOrgJsoupParserCharacterReader_withOrgJsoupParserParseErrorList_(OrgJsoupParserTokeniser *self, OrgJsoupParserCharacterReader *reader, OrgJsoupParserParseErrorList *errors) {
  NSObject_init(self);
  JreStrongAssign(&self->state_, JreLoadEnum(OrgJsoupParserTokeniserState, Data));
  self->isEmitPending_ = false;
  JreStrongAssign(&self->charsString_, nil);
  JreStrongAssignAndConsume(&self->charsBuilder_, new_JavaLangStringBuilder_initWithInt_(1024));
  JreStrongAssignAndConsume(&self->dataBuffer_, new_JavaLangStringBuilder_initWithInt_(1024));
  JreStrongAssignAndConsume(&self->startPending_, new_OrgJsoupParserToken_StartTag_init());
  JreStrongAssignAndConsume(&self->endPending_, new_OrgJsoupParserToken_EndTag_init());
  JreStrongAssignAndConsume(&self->charPending_, new_OrgJsoupParserToken_Character_init());
  JreStrongAssignAndConsume(&self->doctypePending_, new_OrgJsoupParserToken_Doctype_init());
  JreStrongAssignAndConsume(&self->commentPending_, new_OrgJsoupParserToken_Comment_init());
  self->selfClosingFlagAcknowledged_ = true;
  JreStrongAssignAndConsume(&self->charRefHolder_, [IOSCharArray newArrayWithLength:1]);
...
void OrgJsoupParserTokeniser_characterReferenceErrorWithNSString_(OrgJsoupParserTokeniser *self, NSString *message) {
  if ([((OrgJsoupParserParseErrorList *) nil_chk(self->errors_)) canAddError]) [((OrgJsoupParserParseErrorList *) nil_chk(self->errors_)) addWithId:create_OrgJsoupParserParseError_initWithInt_withNSString_withNSObjectArray_([((OrgJsoupParserCharacterReader *) nil_chk(self->reader_)) pos], @"Invalid character reference: %s", [IOSObjectArray arrayWithObjects:(id[]){ message } count:1 type:NSObject_class_()])];
}

void OrgJsoupParserTokeniser_errorWithNSString_(OrgJsoupParserTokeniser *self, NSString *errorMsg) {
  if ([((OrgJsoupParserParseErrorList *) nil_chk(self->errors_)) canAddError]) [((OrgJsoupParserParseErrorList *) nil_chk(self->errors_)) addWithId:create_OrgJsoupParserParseError_initWithInt_withNSString_([((OrgJsoupParserCharacterReader *) nil_chk(self->reader_)) pos], errorMsg)];
}
