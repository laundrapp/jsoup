...
+ (void)initialize {
  if (self == [OrgJsoupHelperStringUtil class]) {
    JreStrongAssignAndConsume(&OrgJsoupHelperStringUtil_padding, [IOSObjectArray newArrayWithObjects:(id[]){ @"", @" ", @"  ", @"   ", @"    ", @"     ", @"      ", @"       ", @"        ", @"         ", @"          " } count:11 type:NSString_class_()]);
...
NSString *OrgJsoupHelperStringUtil_joinWithJavaUtilIterator_withNSString_(id<JavaUtilIterator> strings, NSString *sep) {
  ...
  JavaLangStringBuilder *sb = [create_JavaLangStringBuilder_initWithInt_(64) appendWithNSString:start];
  while ([strings hasNext]) {
    [((JavaLangStringBuilder *) nil_chk(sb)) appendWithNSString:sep];
    [sb appendWithId:[strings next]];
  }
...
NSString *OrgJsoupHelperStringUtil_paddingWithInt_(jint width) {
  OrgJsoupHelperStringUtil_initialize();
  if (width < 0) @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"width must be > 0");
  ...
  IOSCharArray *out = [IOSCharArray arrayWithLength:width];
...
NSString *OrgJsoupHelperStringUtil_normaliseWhitespaceWithNSString_(NSString *string) {
  OrgJsoupHelperStringUtil_initialize();
  JavaLangStringBuilder *sb = create_JavaLangStringBuilder_initWithInt_([((NSString *) nil_chk(string)) java_length]);
...
void OrgJsoupHelperStringUtil_appendNormalisedWhitespaceWithJavaLangStringBuilder_withNSString_withBoolean_(JavaLangStringBuilder *accum, NSString *string, jboolean stripLeading) {
  ...
  for (jint i = 0; i < len; i += JavaLangCharacter_charCountWithInt_(c)) {
    ...
    if (OrgJsoupHelperStringUtil_isWhitespaceWithInt_(c)) {
      ...
      [((JavaLangStringBuilder *) nil_chk(accum)) appendCodePointWithInt:c];
      lastWasWhite = false;
...
JavaNetURL *OrgJsoupHelperStringUtil_resolveWithJavaNetURL_withNSString_(JavaNetURL *base, NSString *relUrl) {
  ...
  if ([relUrl java_indexOf:'.'] == 0 && [((NSString *) nil_chk([((JavaNetURL *) nil_chk(base)) getFile])) java_indexOf:'/'] != 0) {
    base = create_JavaNetURL_initWithNSString_withNSString_withInt_withNSString_([((JavaNetURL *) nil_chk(base)) getProtocol], [base getHost], [base getPort], JreStrcat("C$", '/', [base getFile]));
  }
  return create_JavaNetURL_initWithJavaNetURL_withNSString_(base, relUrl);
}
...
NSString *OrgJsoupHelperStringUtil_resolveWithNSString_withNSString_(NSString *baseUrl, NSString *relUrl) {
  ...
  @try {
    @try {
      base = create_JavaNetURL_initWithNSString_(baseUrl);
    }
    @catch (JavaNetMalformedURLException *e) {
      JavaNetURL *abs = create_JavaNetURL_initWithNSString_(relUrl);
