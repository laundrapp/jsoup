...
+ (void)initialize {
  if (self == [OrgJsoupHelperDataUtil class]) {
    JreStrongAssign(&OrgJsoupHelperDataUtil_charsetPattern, JavaUtilRegexPattern_compileWithNSString_(@"(?i)\\bcharset=\\s*(?:\"|')?([^\\s,;\"']*)"));
    JreStrongAssign(&OrgJsoupHelperDataUtil_mimeBoundaryChars, [@"-_1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ" java_toCharArray]);
...
void OrgJsoupHelperDataUtil_crossStreamsWithJavaIoInputStream_withJavaIoOutputStream_(JavaIoInputStream *inArg, JavaIoOutputStream *outArg) {
  OrgJsoupHelperDataUtil_initialize();
  IOSByteArray *buffer = [IOSByteArray arrayWithLength:OrgJsoupHelperDataUtil_bufferSize];
  jint len;
...
OrgJsoupNodesDocument *OrgJsoupHelperDataUtil_parseByteDataWithJavaNioByteBuffer_withNSString_withNSString_withOrgJsoupParserParser_(JavaNioByteBuffer *byteData, NSString *charsetName, NSString *baseUri, OrgJsoupParserParser *parser) {
  ...
  if (charsetName == nil) {
    ...
    if (meta != nil) {
      ...
      if (foundCharset != nil && [foundCharset java_length] != 0 && ![foundCharset isEqual:OrgJsoupHelperDataUtil_defaultCharset]) {
        ...
        [((JavaNioByteBuffer *) nil_chk(byteData)) rewind];
  ...
  if (doc == nil) {
    doc = [((OrgJsoupParserParser *) nil_chk(parser)) parseInputWithNSString:docData withNSString:baseUri];
    [((OrgJsoupNodesDocument_OutputSettings *) nil_chk([((OrgJsoupNodesDocument *) nil_chk(doc)) outputSettings])) charsetWithNSString:charsetName];
  }
  return doc;
}
...
JavaNioByteBuffer *OrgJsoupHelperDataUtil_readToByteBufferWithJavaIoInputStream_withInt_(JavaIoInputStream *inStream, jint maxSize) {
  ...
  IOSByteArray *buffer = [IOSByteArray arrayWithLength:OrgJsoupHelperDataUtil_bufferSize];
  JavaIoByteArrayOutputStream *outStream = create_JavaIoByteArrayOutputStream_initWithInt_(OrgJsoupHelperDataUtil_bufferSize);
  jint read;
...
JavaNioByteBuffer *OrgJsoupHelperDataUtil_readFileToByteBufferWithJavaIoFile_(JavaIoFile *file) {
  ...
  @try {
    ...
    IOSByteArray *bytes = [IOSByteArray arrayWithLength:(jint) [randomAccessFile length]];
...
NSString *OrgJsoupHelperDataUtil_mimeBoundary() {
  OrgJsoupHelperDataUtil_initialize();
  JavaLangStringBuilder *mime = create_JavaLangStringBuilder_initWithInt_(OrgJsoupHelperDataUtil_boundaryLength);
  JavaUtilRandom *rand = create_JavaUtilRandom_init();
  for (jint i = 0; i < OrgJsoupHelperDataUtil_boundaryLength; i++) {
    [mime appendWithChar:IOSCharArray_Get(OrgJsoupHelperDataUtil_mimeBoundaryChars, [rand nextIntWithInt:((IOSCharArray *) nil_chk(OrgJsoupHelperDataUtil_mimeBoundaryChars))->size_])];
  }
