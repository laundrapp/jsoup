...
- (NSString *)getPlainTextWithOrgJsoupNodesElement:(OrgJsoupNodesElement *)element {
  OrgJsoupExamplesHtmlToPlainText_FormattingVisitor *formatter = create_OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_initWithOrgJsoupExamplesHtmlToPlainText_(self);
  OrgJsoupSelectNodeTraversor *traversor = create_OrgJsoupSelectNodeTraversor_initWithOrgJsoupSelectNodeVisitor_(formatter);
...
void OrgJsoupExamplesHtmlToPlainText_mainWithNSStringArray_(IOSObjectArray *args) {
  ...
  OrgJsoupExamplesHtmlToPlainText *formatter = create_OrgJsoupExamplesHtmlToPlainText_init();
  if (selector != nil) {
...
- (void)headWithOrgJsoupNodesNode:(OrgJsoupNodesNode *)node
  ...
  else if (OrgJsoupHelperStringUtil_inWithNSString_withNSStringArray_(name, [IOSObjectArray arrayWithObjects:(id[]){ @"p", @"h1", @"h2", @"h3", @"h4", @"h5", @"tr" } count:7 type:NSString_class_()])) OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_appendWithNSString_(self, @"\n");
}

- (void)tailWithOrgJsoupNodesNode:(OrgJsoupNodesNode *)node
                          withInt:(jint)depth {
  NSString *name = [((OrgJsoupNodesNode *) nil_chk(node)) nodeName];
  if (OrgJsoupHelperStringUtil_inWithNSString_withNSStringArray_(name, [IOSObjectArray arrayWithObjects:(id[]){ @"br", @"dd", @"dt", @"p", @"h1", @"h2", @"h3", @"h4", @"h5" } count:9 type:NSString_class_()])) OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_appendWithNSString_(self, @"\n");
  else if ([((NSString *) nil_chk(name)) isEqual:@"a"]) OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_appendWithNSString_(self, NSString_java_formatWithNSString_withNSObjectArray_(@" <%s>", [IOSObjectArray arrayWithObjects:(id[]){ [node absUrlWithNSString:@"href"] } count:1 type:NSObject_class_()]));
}
...
void OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_initWithOrgJsoupExamplesHtmlToPlainText_(OrgJsoupExamplesHtmlToPlainText_FormattingVisitor *self, OrgJsoupExamplesHtmlToPlainText *outer$) {
  NSObject_init(self);
  self->width_ = 0;
  JreStrongAssignAndConsume(&self->accum_, new_JavaLangStringBuilder_init());
}
...
void OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_appendWithNSString_(OrgJsoupExamplesHtmlToPlainText_FormattingVisitor *self, NSString *text) {
  ...
  if ([text isEqual:@" "] && ([((JavaLangStringBuilder *) nil_chk(self->accum_)) java_length] == 0 || OrgJsoupHelperStringUtil_inWithNSString_withNSStringArray_([((JavaLangStringBuilder *) nil_chk(self->accum_)) substringWithInt:[self->accum_ java_length] - 1], [IOSObjectArray arrayWithObjects:(id[]){ @" ", @"\n" } count:2 type:NSString_class_()]))) return;
  if ([text java_length] + self->width_ > OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_maxWidth) {
    ...
    for (jint i = 0; i < ((IOSObjectArray *) nil_chk(words))->size_; i++) {
      ...
      if ([((NSString *) nil_chk(word)) java_length] + self->width_ > OrgJsoupExamplesHtmlToPlainText_FormattingVisitor_maxWidth) {
        [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk(self->accum_)) appendWithNSString:@"\n"])) appendWithNSString:word];
        self->width_ = [word java_length];
      }
      else {
        [((JavaLangStringBuilder *) nil_chk(self->accum_)) appendWithNSString:word];
        self->width_ += [word java_length];
      }
    }
  }
  else {
    [((JavaLangStringBuilder *) nil_chk(self->accum_)) appendWithNSString:text];
